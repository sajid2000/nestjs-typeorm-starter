...
import { CursorPaginationResponse } from "./@types/classes/cursor-pagination.response";
import { OffsetMeta, OffsetPaginationResponse } from "./@types/classes/offset-pagination.response";
import { CursorPaginationOptions, OffsetPaginationOptions } from "./@types/pagination.type";
import { CustomBaseEntity } from "./base.entity";

export class BaseRepository<TEntity extends CustomBaseEntity> extends Repository<TEntity> {
  ...
  async cursorPagination(dto: CursorPaginationOptions<TEntity>) {
    ...
    if (cursor) {
      ...
      whereQuery[cursorField as string] = order === "asc" ? MoreThan(decodeCursor) : LessThan(decodeCursor);
    }
    ...
    return {
      ...
      meta: {
        nextCursor: count > 0 ? this.encodeCursor(data[count - 1]![cursorField as string]) : null,
  ...
  async offsetPagination(dto: OffsetPaginationOptions<TEntity>) {
    const { fields, limit, order, orderBy, relations, searchField, withDeleted, fromDate, search, toDate } = dto;
    ...
    const pageMeta = new OffsetMeta({ itemCount, pageOptionsDto: dto });
  ...
  protected decodeCursor(cursor: string, cursorType: "string" | "number" | "date" = "string"): string | number | Date {
    ...
    switch (cursorType) {
      case "date": {
        ...
        if (Number.isNaN(millisUnix)) throw new BadRequestException("Cursor Invalid Date");
      ...
      case "number": {
        ...
        if (Number.isNaN(number)) throw new BadRequestException("Cursor Invalid Number");
