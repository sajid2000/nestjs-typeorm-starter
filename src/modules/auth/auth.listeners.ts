...
import { appConfig } from "@/lib/config/configs";
...
export class AuthEventListener {
  ...
  async handleUserRegistered({ email, token }: UserRegisteredEvent) {
    const link = `${this.appConfigs.clientUrl}/verify-email?token=${token}`;
  ...
  async handleSendResetLink({ email, token }: SendPasswordResetLinkEvent) {
    const link = `${this.appConfigs.clientUrl}/new-password?token=${token}`;
