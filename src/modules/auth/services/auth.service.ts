...
import { AccessTokenPayload, AuthResponse } from "@/common/@types";
import { ValidationException } from "@/common/exceptions/validation.exception";
import { authConfig } from "@/lib/config/configs";
import { UserService } from "@/modules/user/user.service";

import { ResendVerifyLinkEvent, SendPasswordResetLinkEvent, UserRegisteredEvent } from "../auth.events";
import { RESEND_VERIFY_LINK, SEND_PASSWORD_RESET_LINK, USER_REGISTERED } from "../auth.listeners";
...
export class AuthService {
  ...
  async resendVerifyLink(email: string) {
    ...
    const prevToken = await this.cacheManager.get(`verify-token.email=${email}`);
  ...
  async verifyEmail(token: string) {
    try {
      ...
      const cachedVerifyToken = await this.cacheManager.get(`verify-token.email=${email}`);
  ...
  async login(email: string, password: string) {
    try {
      ...
      if (!user.password) throw new ValidationException("email", "User not found");

      if (!user.isVerified) {
        return await this.resendVerifyLink(email);
      }
      ...
      if (error instanceof NotFoundException) throw new ValidationException("email", "User not found");
  ...
  async refreshAccessToken(refreshToken: string) {
    try {
      const { email } = this.jwtService.verify<AccessTokenPayload>(refreshToken, {
  ...
  async sendPasswordResetLink(email: string) {
    const cachedToken = await this.cacheManager.get(`reset-password-token.email=${email}`);
    ...
    try {
      ...
      if (!user.isVerified) throw new ValidationException("email", "Email not verified!");
